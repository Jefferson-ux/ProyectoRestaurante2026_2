...
CREATE TABLE tipo_contrato (
    ...
    CHECK (TRIM(nombre_tipo_contrato) <> '')
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE cliente (
    ...
    estado              TINYINT DEFAULT 1 NOT NULL,
    CHECK (dni_cliente REGEXP '^[0-9]{8}$'),
    CHECK (estado IN (0,1))
...
CREATE TABLE empleado (
    ...
    CHECK (TRIM(nombre_empleado) <> ''),
    CHECK (TRIM(apellido_empleado) <> ''),
    CHECK (dni_empleado REGEXP '^[0-9]{8}$'),
    CHECK (correo_principal REGEXP '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}$'),
    CHECK (telefono_principal REGEXP '^[0-9]{7,15}$'),
    CHECK (estado IN (0,1))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE contrato (
    ...
    fecha_contrato          DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL,
