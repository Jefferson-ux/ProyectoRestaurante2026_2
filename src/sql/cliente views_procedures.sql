...
/* ============================================================
   5. PROCEDURE REACTIVAR
   ============================================================ */
DROP PROCEDURE IF EXISTS PROC_ReactivarUsuario;
DELIMITER $$

CREATE PROCEDURE PROC_ReactivarUsuario (
    IN p_id_usuario INT
)
BEGIN
    DECLARE v_existe INT DEFAULT 0;

    -- Validar si el usuario existe y está inactivo (estado = 0)
    SELECT COUNT(*) INTO v_existe
    FROM usuario
    WHERE id_usuario = p_id_usuario AND estado = 0;
    ...
    ELSE
        SELECT 'No se encontró un usuario inactivo con ese ID.' AS mensaje;
    END IF;
...
/* ============================================================
   5. PROCEDURE VALIDAR
   ============================================================ */
DROP PROCEDURE IF EXISTS PROC_ValidarLogin;
DELIMITER $$

CREATE PROCEDURE PROC_ValidarLogin (
    IN p_codigo VARCHAR(15),
    IN p_password_encriptada VARCHAR(255),
    IN p_nombre_cargo VARCHAR(85)
)
BEGIN
    -- Buscar usuario con los criterios ingresados
    SELECT 
        u.id_usuario,
        u.codigo_usuario,
        c.nombre_cargo,
    ...
    WHERE UPPER(u.codigo_usuario) = UPPER(TRIM(p_codigo))
      AND u.password_usuario = TRIM(p_password_encriptada)
      AND UPPER(c.nombre_cargo) = UPPER(TRIM(p_nombre_cargo))
      AND u.estado = 1;
